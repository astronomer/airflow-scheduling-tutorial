...
class UnevenIntervalsTimetable(Timetable):
    ...
    def infer_manual_data_interval(self, run_after: DateTime) -> DataInterval:
        ...
        if run_after >= run_after.set(hour=6, minute=0) and run_after <= run_after.set(hour=16, minute=30):
            start = (run_after-delta).set(hour=16, minute=30).replace(tzinfo=UTC)
            end = run_after.set(hour=6, minute=0).replace(tzinfo=UTC)
        # If time is after 16:30 but before midnight, period is between 6:00 and 16:30 the same day
            ...
            start = run_after.set(hour=6, minute=0).replace(tzinfo=UTC)
            end = run_after.set(hour=16, minute=30).replace(tzinfo=UTC)
